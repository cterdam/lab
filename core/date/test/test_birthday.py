...
class BirthdayTest(unittest.TestCase):
    def test_chinese_new_year_anchor(self):
        ...
        self.assertEqual(
            birthday.infer(1999, chinese="正月初一")["birthday_gregorian"], "02-16"
        )

    def test_infer_from_chinese(self):
        got = birthday.infer(1999, chinese="八月十一")
        self.assertEqual(got["birthday_gregorian"], "09-20")
        self.assertEqual(got["zodiac"], "Virgo")
        self.assertEqual(got["chinese_zodiac"], "Rabbit")

    def test_infer_from_gregorian(self):
        got = birthday.infer(1998, gregorian="09-06")
        self.assertEqual(got["birthday_chinese"], "七月十六")
        self.assertEqual(got["chinese_zodiac"], "Tiger")

    def test_late_lunar_month_falls_in_next_solar_year(self):
        # Lunar 1999-12-20 lands in solar 2000; birth_year 2000 must resolve
        # to lunar year 1999 (a Rabbit, not a Dragon).
        got = birthday.infer(2000, chinese="十二月二十")
        self.assertEqual(got["birthday_gregorian"], "01-26")
        self.assertEqual(got["chinese_zodiac"], "Rabbit")
    ...
    def test_zodiac_boundaries(self):
        self.assertEqual(birthday.zodiac("01-19"), "Capricorn")
        self.assertEqual(birthday.zodiac("01-20"), "Aquarius")
        self.assertEqual(birthday.zodiac("12-22"), "Capricorn")
