...
from src.lib.rng.dice import Dice
...
def test_is_rng_subclass():
    """Dice is a subclass of RNG."""
    assert issubclass(Dice, RNG)


def test_instance_of_rng():
    """Dice instance is also an RNG."""
    d = Dice(seed=1, logname="test_isinstance")
    assert isinstance(d, RNG)


def test_logspace():
    """logspace includes both 'rng' and 'dice'."""
    d = Dice(seed=1, logname="test_logspace")
...
def test_default_6_sides():
    ...
    d = Dice(seed=1, logname="test_default")
...
def test_custom_sides():
    ...
    d = Dice(20, seed=1, logname="test_d20")
...
def test_d4():
    """4-sided die."""
    d = Dice(4, seed=1, logname="test_d4")
...
def test_d12():
    """12-sided die."""
    d = Dice(12, seed=1, logname="test_d12")
...
def test_roll_returns_int():
    ...
    d = Dice(seed=1, logname="test_roll")
    face = d.roll()
...
def test_roll_in_range():
    ...
    d = Dice(seed=1, logname="test_roll_range")
    face = d.roll()
    assert 1 <= face <= 6
...
def test_roll_decrements_remaining():
    ...
    d = Dice(seed=1, logname="test_roll_rem")
    d.roll()
...
def test_roll_all_faces():
    ...
    d = Dice(seed=1, logname="test_roll_all")
...
def test_roll_exhausted():
    ...
    d = Dice(seed=1, logname="test_roll_empty")
    for _ in range(6):
        d.roll()
...
def test_roll_after_reshuffle():
    ...
    d = Dice(seed=1, logname="test_roll_resh")
    for _ in range(6):
        d.roll()
    d.reshuffle()
    assert d.remaining == 6
...
def test_deterministic():
    ...
    d1 = Dice(seed=42, logname="test_det1")
    d2 = Dice(seed=42, logname="test_det2")
...
def test_different_seeds_differ():
    ...
    d1 = Dice(20, seed=1, logname="test_diff1")
    d2 = Dice(20, seed=999, logname="test_diff2")
...
def test_load_rejects_non_int():
    ...
    d = Dice(seed=1, logname="test_load_bad")
    d.load(["a", "b", "c"])
...
def test_load_rejects_negative():
    ...
    d = Dice(seed=1, logname="test_load_neg")
    d.load([-1, 0, 1])
    assert d.remaining == 6
...
def test_load_rejects_zero():
    """load() rejects zero."""
    d = Dice(seed=1, logname="test_load_zero")
    d.load([0, 1, 2])
    assert d.remaining == 6
...
def test_load_rejects_float():
    """load() rejects floats."""
    d = Dice(seed=1, logname="test_load_float")
    d.load([1.5, 2.5])
    assert d.remaining == 6
...
def test_load_accepts_valid():
    ...
    d = Dice(seed=1, logname="test_load_good")
...
def test_draw_works():
    """draw() still works on Dice."""
    d = Dice(seed=1, logname="test_draw")
    face = d.draw()
    assert isinstance(face, int)
    assert 1 <= face <= 6
...
def test_randint_works():
    """Plain RNG methods still work on Dice."""
    d = Dice(seed=42, logname="test_randint")
    val = d.randint(1, 100)
    assert 1 <= val <= 100
...
def test_cleanup():
    """Dice is garbage collected cleanly."""
    d = Dice(seed=1, logname="test_gc")
    weak = weakref.ref(d)
    del d
    gc.collect()
    assert weak() is None
...
def test_yahtzee_scenario():
    ...
    dice = [Dice(seed=i, logname=f"test_yah_{i}") for i in range(5)]
